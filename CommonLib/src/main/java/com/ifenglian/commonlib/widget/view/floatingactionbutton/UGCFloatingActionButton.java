...
public class UGCFloatingActionButton extends AppCompatImageButton {
    ...
    public UGCFloatingActionButton(Context context) {
        this(context, null);
    }

    public UGCFloatingActionButton(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public UGCFloatingActionButton(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(Context context, AttributeSet attributeSet) {
        mVisible = true;
        mColorNormal = Color.parseColor("#5677fc");
        ...
        if (hasLollipopApi()) {
            StateListAnimator stateListAnimator = AnimatorInflater.loadStateListAnimator(context,
                    R.drawable.ugc_fab_press_elevation);
    ...
    private void initAttributes(Context context, AttributeSet attributeSet) {
        TypedArray attr = getTypedArray(context, attributeSet, R.styleable.UGCFloatingActionButton);
        if (attr != null) {
            try {
                mColorNormal = attr.getColor(R.styleable.UGCFloatingActionButton_fab_colorNormal,
                        Color.parseColor("#5677fc"));
                mColorPressed = attr.getColor(R.styleable.UGCFloatingActionButton_fab_colorPressed,
                        darkenColor(mColorNormal));
                mColorRipple = attr.getColor(R.styleable.UGCFloatingActionButton_fab_colorRipple,
                        lightenColor(mColorNormal));
                mColorDisabled = attr.getColor(R.styleable.UGCFloatingActionButton_fab_colorDisabled,
                        mColorDisabled);
                mShadow = attr.getBoolean(R.styleable.UGCFloatingActionButton_fab_shadow, true);
                mType = attr.getInt(R.styleable.UGCFloatingActionButton_fab_type, TYPE_NORMAL);
            } finally {
                attr.recycle();
            }
        }
    ...
    private Drawable createDrawable(int color) {
        ...
        if (mShadow && !hasLollipopApi()) {
            Drawable shadowDrawable = getResources().getDrawable(mType == TYPE_NORMAL ? R.mipmap.ugc_fab_shadow
                    : R.mipmap.ugc_fab_shadow_mini);
    ...
    public void attachToListView(@NonNull AbsListView listView,
                                 UGCScrollDirectionListener scrollDirectionListener) {
    ...
    public void attachToScrollView(@NonNull UGCObservableScrollView scrollView) {
    ...
    public void attachToScrollView(@NonNull UGCObservableScrollView scrollView,
                                   UGCScrollDirectionListener scrollDirectionListener,
                                   UGCObservableScrollView.OnScrollChangedListener onScrollChangedListener) {
    ...
    private class AbsListViewScrollDetectorImpl extends UGCAbsListViewScrollDetector {
        private UGCScrollDirectionListener mScrollDirectionListener;
        private AbsListView.OnScrollListener mOnScrollListener;

        private void setScrollDirectionListener(UGCScrollDirectionListener scrollDirectionListener) {
    ...
    private class RecyclerViewScrollDetectorImpl extends UGCRecyclerViewScrollDetector {
        private UGCScrollDirectionListener mScrollDirectionListener;
    ...
    private class ScrollViewScrollDetectorImpl extends UGCScrollViewScrollDetector {
        private UGCScrollDirectionListener mScrollDirectionListener;

        private UGCObservableScrollView.OnScrollChangedListener mOnScrollChangedListener;

        private void setScrollDirectionListener(UGCScrollDirectionListener scrollDirectionListener) {
            mScrollDirectionListener = scrollDirectionListener;
        }

        public void setOnScrollChangedListener(UGCObservableScrollView.OnScrollChangedListener onScrollChangedListener) {
